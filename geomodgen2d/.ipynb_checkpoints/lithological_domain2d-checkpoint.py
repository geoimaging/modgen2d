...
# Merged lithological
    ...
    def plot2d(self, ax=None, 
        ...
        cax = ax.imshow(integer_mapped_array, cmap=fixed_cmap, extent=[0, self.span_x, self.span_z, 0], interpolation='nearest')
        ax.set(xlim=[0, self.span_x], ylim = [self.span_z, 0])
...
class LithologicalDomain2D(LithologicalDomain2DFunctions):
    ...
    def add_surface_boundary_to_curr_boundary(self, SurfaceBoundaryCreator_class, method='pile'):
        ...
        self.lm_type = 'from_boundary'
        self.surface_boundary = SurfaceBoundaryCreator_class.boundary_array
    ...
    def merge_with_Utils2D_domains(self, list_lithologicalDomainFromUtils2D = None): #2D NOT CORRECTED YET
        if list_lithologicalDomainFromUtils2D is not None:
            ...
            for i in range(len(list_lithologicalDomainFromUtils2D)):
                if i==0:
                    mergedUtilsDomain = list_lithologicalDomainFromUtils2D[0]
                else:
                    mergedUtilsDomain = merge_two_lithological_domain(mergedUtilsDomain, list_lithologicalDomainFromUtils2D[i], added_prefix=None)
...
def merge_two_lithological_domain(lithologicalDomainA, lithologicalDomainB, added_prefix=None):
    ...
    assert np.prod(lithologicalDomainA.x_ranges == lithologicalDomainB.x_ranges)==1, f"Expected del_x = del_x from class, but provided {lithologicalDomainA.del_x} != {Utils_to_actual_grid_class_other.del_x}"
    assert np.prod(lithologicalDomainA.z_ranges == lithologicalDomainB.z_ranges)==1, f"Expected del_z = del_z from class, but provided {lithologicalDomainA.del_z} != {Utils_to_actual_grid_class_other.del_z}"
