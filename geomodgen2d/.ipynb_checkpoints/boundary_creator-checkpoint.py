...
class BoundaryCreator(Domain2D):
    ...
    def filtering_boundary(self, filter_window=21, filter_order=3):
        """
        ...
        if self.dim == 2:  #If 2D only.
            ...
            if filter_window!=0:
                b_array = scipy.signal.savgol_filter(self.boundary_array, window_length=filter_window, polyorder=filter_order, axis=1)#, window_length, polyorder
                self.edit_boundary_matrix(b_array)
    ...
    def remeshing_boundary(self, new_del_x, interp_method='linear'):
        ...
        if self.dim == 1:
            ...
            self._x_ranges = remeshed_3D_domain.x_ranges

            if n_interface!=0:
                zs = range(n_interface)
                b_array = f.remeshing_2D_matrix(x_old = x_ranges_old, x_new = self.x_ranges, z_old = zs, z_new = zs, matrix_2d = self.boundary_array.T, interp_method = interp_method)
                self.boundary_array = b_array.T
