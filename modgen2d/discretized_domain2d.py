...
class DiscretizedDomain2D():
    ...
    def __init__(self, span_x: float, span_z: float, dx: float, dz: float, length_config:LengthConfig = None):
        """
        ...
        if not self.is_valid_mesh(self._spans_in_domain_len_units, self._dhs_in_domain_len_units):
            msg = f"Requirements: spans and dels must be positive and spans in domain_units (where 1 {length_config.physical_length_unit} = {length_config.max_grid_density} domain_units)={self._spans_in_domain_len_units} must be divisible by dhs in domain_units={self._dhs_in_domain_len_units}."
            raise ValueError(msg)
        
        # define centers of each element
        self.x_centers = (np.arange(0, self._spans_in_domain_len_units[0], self._dhs_in_domain_len_units[0]) + self._dhs_in_domain_len_units[0] / 2)/length_config.max_grid_density
        self.z_centers = (np.arange(0, self._spans_in_domain_len_units[1], self._dhs_in_domain_len_units[1]) + self._dhs_in_domain_len_units[1] / 2)/length_config.max_grid_density
    ...
    def remesh(self, new_dx:float = None, new_dz:float = None):#, inplace=True):
        ...
        return DiscretizedDomain2D(self.spans[0], self.spans[1], new_dx, new_dz, self.length_config)
    ...
    def get_minimum_domain(domain2d_list:list):
        ...
        return DiscretizedDomain2D(base_domain.spans[0], base_domain.spans[1],
                                   min_dh_x, min_dh_z, base_domain.length_config)
    ...
    def from_config(cls, config_dict):
        ...
        try:
            ...
            return cls(*spans, *dhs, length_config)
